...
//        $(this).css('width', options.width);
		...
		if ($.isArray(cOptions.yAxis)){
			...
			for(var i in cOptions.yAxis){
				formatYAxis(cOptions.yAxis[i]);
			}
		}else{
			mainYAxis = cOptions.yAxis;
			formatYAxis(cOptions.yAxis);
		}		
...
//                    symbolWidth: 12 
    //                align: 'left'
		...
		function formatYAxis(yAxis){
			...
			if (yAxis.dataFormat == 3 && maxY > 95){
				yAxis.max = 100;
			}
			if (yAxis.dataFormat == 5 && maxY > 0.95){
				yAxis.max = 1;
			}
            //如果所有数据都为0，则Y值默认取100
            if (maxY == 0){
...
*/
        ...
        for(var i in hcData){
            var point = hcData[i];
            if (point.y != null){
                options.theme != 'wechat' && isNotPieChart() && (point.marker.enabled = counter <= 7);
