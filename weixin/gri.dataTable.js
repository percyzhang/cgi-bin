...
(function (global, document, undefined) {
    ...
    DataTable.fn = DataTable.prototype = {
            ...
            for (var i = 0; i < this.page.data.length; i++) {
                ...
                for (var name in allFields) {
                    ...
                    //如果是时间类型，根据指定的格式进行格式化
                        ...
                        fieldData = this.timeFormat(fieldData);
                    }
        ...
        createLitePage:function() {
            //当前对象的引用传递给_this
            var _this = this;

            if (this.page) {
                var page_right_div = document.createElement('span');
                page_right_div.className = 'page_nav_area';
                this.page.container.appendChild(page_right_div);
            }
            ...
            spanNum.className = 'page_num';
            var tempIndex = _this.page.index + 1;
            if (tempIndex == _this.page.count) {
                tempIndex = _this.page.count;
            }
            spanNum.innerHTML = ('<label>'+tempIndex+'</label>'+"\n"+'<span class="num_gap">/</span>'+"\n"+'<label>'+this.page.count+'</label>');
            page_right_div.appendChild(spanNum);

            //如果page的页数大于page的偏移量
            if (this.page.count - _this.page.index >1) {
            ...
            spanGoto.className = 'goto_area';
            spanGoto.innerHTML = '<input type="text">'+"\n";

            var aGo = document.createElement('A');
            aGo.className = 'button page_go';
            aGo.innerHTML = '跳转';
            aGo.setAttribute('href', 'javascript:;');
            this.addEvent(aGo, 'click', function () {
                var tempIndex = spanGoto.getElementsByTagName('input')[0].value - 1;
                if (tempIndex >= _this.page.count || tempIndex<0)  {
                    spanGoto.getElementsByTagName('input')[0].value = '';
                    return false;
                }
                _this.page.index = tempIndex;
                if (_this.page.ifRealPage) {
                    _this.createJsonTable(_this);
                } else {
                    _this.createTable(_this.options);
            ...
            this.addEvent(spanGoto.getElementsByTagName('input')[0], 'keydown', function() {
                var e = e || window.event;
                if(e.keyCode == 13){
                    aGo.click();
                }
            });
            spanGoto.appendChild(aGo);
            page_right_div.appendChild(spanGoto);
        },
        /**
         * 创建分页控件
        ...
        createPage:function () {
            ...
            if (this.page.index < this.page.count) {
                ...
                if (this.page.theme == 'lite') {
                    this.page.container.className = this.page.containerClass || 'pagination pagination_wrp';
                    this.page.nextClass = this.page.nextClass || 'button page_next';
                    this.page.prevClass = this.page.prevClass || 'button page_prev';

                    this.createLitePage();
                } else {
                    this.page.container.className = this.page.containerClass || 'gri_pg';
                    this.page.nextClass = this.page.nextClass || 'button next';
        ...
        createComplexHeader:function (complexHeader, table, _this) {
            ...
            for (var name in complexHeader) {
                ...
                for (var i = 0; i < _tr.length; i++) {
                    ...
                    tH.style.border = "1px solid #e8e8e8";
